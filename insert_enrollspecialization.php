...
// if you enrolling for first time
	if(empty($row[0])){
    	$query="UPDATE first_second_year_student_details SET student_specialization = '$student_specialization' WHERE student_rollnno = '$student_rollnno'";
    	
    	$result=mysqli_query($con,$query);
    	$queryu="UPDATE second_year_students SET student_specialization = '$student_specialization' WHERE student_rollno = '$student_rollnno'";
		...
		$query="UPDATE first_second_year_student_details SET student_specialization = '$finalvalue' WHERE student_rollnno = '$student_rollnno'";
    	$result=mysqli_query($con,$query);
		
		$queryu="UPDATE second_year_students SET student_specialization = '$finalvalue' WHERE student_rollno = '$student_rollnno'";
            $resultu=mysqli_query($con,$queryu);
