...
$sqlsecond="SELECT * FROM course_details WHERE course_details_name = '".$course_details_name."';";
...
if(mysqli_num_rows($resultfirst) > 0){
    $query = "DELETE FROM first_year_course_list WHERE first_year_course_list_name = '".$course_details_name."';";
     $queryupdate ="UPDATE first_year SET `first_year_courses`=REPLACE(first_year_courses,'".$course_details_name."','')";
     $result = mysqli_query($con,$queryupdate);
}

else{
    $query = "DELETE FROM course_details WHERE course_details_name = '".$course_details_name."';";
}
