	...
	<div class="row">
		...
		<div class="col-sm-8">
				<form role="form" action="insert_course_schedule_details_firstyear.php" method="post">
					...
					<div class="form-group">
						<label>Course Code</label><select name="course_code">
						<?php
                            require "init.php";
                            $sql = mysqli_query($con, "SELECT first_year_course_list_code FROM first_year_course_list");
                            while ($row = $sql->fetch_assoc()){
                            echo '<option value="'.$row['first_year_course_list_code'].'">' . $row['first_year_course_list_code'] . "</option>";
                            }
                         ?>
                         </select>
					</div>
					...
					<div class="form-group">
						<label>Faculty Code</label><select name="faculty_code">
                        <?php
                            require "init.php";
                            $sql = mysqli_query($con, "SELECT faculty_lastname,faculty_firstname,faculty_code FROM faculty_details");
                            while ($row = $sql->fetch_assoc()){
                            echo '<option value="'.$row['faculty_code'].'">' . $row['faculty_code'] . "</option>";
                            }
                         ?>
                          </select>
					</div>
					<div class="form-group">
						<label>Course Date (dd-mm-yyyy)</label>
						<input type="text" name="course_date" class="form-control" />
					</div>
					<div class="form-group">
						<label>Course Time (00:00-00:00 AM/PM)</label>
						<input type="text" name="course_schedule_time" class="form-control" />
					</div>
					<div class="form-group">
						<label>Student Group</label>
						<input type="text" name="student_group" class="form-control" value="0"/>
					</div>
					<div class="form-group">
