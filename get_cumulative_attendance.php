...
$querysection= "SELECT batch_number FROM first_year WHERE student_rollnno='$student_rollnno'";
     ...
     $queryfeedback= "SELECT student_id,course_date,course_time FROM attendace_after_feedback WHERE course_name='$course_details_name'";
      $resultfeedback = mysqli_query($con,$queryfeedback);
        ...
        while($row=mysqli_fetch_array($resultfeedback)){
           	$queryattendance= "SELECT student_rollnno FROM attendace_details_first WHERE (attendance_date='$row[1]' AND attendance_time='$row[2]'AND attendance_status='Present' AND section_name='$sectionval[batch_number]')"; 
        ...
        while($row=mysqli_fetch_array($resultfeedback)){
        	$queryattendance= "SELECT student_rollnno FROM attendace_details WHERE (attendance_date='$row[1]' AND attendance_time='$row[2]'AND attendance_status='Present')";
        	$resultattendancek = mysqli_query($con,$queryattendance);
        	while($rowa=mysqli_fetch_array($resultattendancek)){
        	    	$myarray1 = array_filter(array_map('trim', explode(',', $rowa[0])));
                	$myarray2 = array_filter(array_map('trim', explode(',', $row[0])));
...
foreach($response as $key=>$value1) {
    foreach($value1 as $key=>$value) {
        ...
        if($value==="Feedback Pending"){
           $feedback_counter+=1;
        }
    }
}
//echo json_encode(array('cumulative'=>$response));
array_push($responseresult,array("Total_count"=>sizeof($response),"Present_count"=>$present_counter,"Absent_count"=>$absent_counter,"Feedback_count"=>$feedback_counter,"Percentage_count"=>(round(($present_counter*100)/(sizeof($response)),2))."%" ));
echo json_encode(array('cumulative'=>$responseresult));
